function [p_angles,iterations,bvecs]=threeR(n,k,data,initvecs,SBSP,swtch)
L = length(data);
...
for i=1:L
    if mod(i,100)==0
        perc=100*i/(L);
        perc=round(perc);
        %string=sprintf('%d percent of algorithm 3R, with switch %d, complete', perc,swtch);
        %disp(string)
...
inexpM = vectors * diag(log(values)) * vectors' - eta* ( newx*newx'* Mprev +Mprev*newx*newx')/ (2*norm(Mprev*newx)) ;
...
if ~isequal(size(SBSP), size(bvecs))
